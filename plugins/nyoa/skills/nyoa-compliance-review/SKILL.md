...
1. **Read agent identity.** Load `nyoa-context/profile.md` and extract `license_state`, `brokerage`, and `nar_member`. If `nyoa-context/profile.md` is missing, blank, or any of these fields is unset, warn the user — "I couldn't find your `license_state` in `nyoa-context/profile.md`, so I'm running federal-only review. Run `/nyoa-setup identity` to add it and I'll apply your state's rules on the next review." — then continue with federal-only review.
...
8. **Write the audit-log entry.** Append one line to `nyoa-workspace/compliance-log.md` in this exact format:
   ...
   - If `nyoa-workspace/compliance-log.md` does not exist, create it with the header below (matches `plugins/nyoa/assets/workspace-template/compliance-log.md` seeded by `/nyoa-setup`), then append the entry:
     ```
     # Compliance Log
...
**Reads:**
- `nyoa-context/profile.md` — `license_state`, `brokerage`, `nar_member`, `license_number` (used in audit log note when relevant)
